...
const CertBR = ({ certificateData, goBack }) => {
  const [isLoading, setLoading] = useState(false);
  const [isValid, setIsValid] = useState(false);

  useEffect(() => {
    setLoading(true);
    const url = certificateData.substring(0, 75);

    if ((url.includes("https://www.healthapp.gov.bn/covid19/bruhealth/vaccines_new.html")) ||
    (url.includes("https://www.healthinfo.gov.bn/covid19/bruhealth/vaccines_new.html")) ){
      setIsValid(true);
      setLoading(false);
    } else {
      setIsValid(false);
